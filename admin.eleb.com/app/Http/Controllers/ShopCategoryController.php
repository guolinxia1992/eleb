...
class ShopCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request,
            ['name'=>'required','img'=>'required|image'],
            ['name.required'=>'请输入分类名称','img.required'=>'请上传图片','img.image'=>'图片格式有误']);
        //数据无误保存
        $img = $request->file('img');
        $path = $img->store('public/shopcategory');
        ShopCategory::create(['name'=>$request->name,'img'=>$path,'status'=>$request->status]);
        //数据写入session
