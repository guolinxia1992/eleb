...
<div class="x-body layui-anim layui-anim-up" >
    ...
    @endif
    <form class="layui-form" method="post" action="{{route('roles.update',[$role])}}" enctype="multipart/form-data">
        ...
        <div class="layui-form-item">
            <label for="" class="layui-form-label" style="width: 120px;">
                <span class="x-red">*</span>permissions
            </label>
            <div class="">
                @foreach($permissions as $permission)
                <input type="checkbox"  name="permission[]"  autocomplete="off" class="" value="{{$permission->name}}" @if($role->hasPermissionTo($permission->name)) checked @endif>{{$permission->name}}
                @endforeach
            </div>
        </div>
